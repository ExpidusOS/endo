...
#include <endo-desktop-item-edit/endo-die-utils.h>
...
static void endo_die_error (const gchar *format, ...) G_GNUC_PRINTF (1, 2);
...
static void
endo_die_error (const gchar *format,
               ...)
{
  gchar   *buffer;
  va_list  args;
...
static void
endo_die_response_cb (GtkDialog *dialog,
                     gint       response_id,
                     gpointer   user_data)
{
  if (response_id == GTK_RESPONSE_HELP)
    expidus_dialog_show_help (GTK_WINDOW (dialog), "endo", "desktop-item-edit", NULL);
}
...
main (int argc, char **argv)
{
      ...
      if (G_UNLIKELY (error != NULL))
        {
          ...
          endo_die_error ("%s", error->message);
          g_error_free (error);
        }
      else
        {
          /* no error message, the GUI initialization failed */
          endo_die_error ("%s %s", _("Failed to open display"),
  ...
  if (G_UNLIKELY (argc != 2))
    {
      endo_die_error (_("No file/folder specified"));
      return EXIT_FAILURE;
    }
  ...
  if (opt_create_new)
    {
      ...
      /* type specific stuff */
      if (endo_str_is_equal (opt_type, G_KEY_FILE_DESKTOP_TYPE_LINK))
        {
          g_key_file_set_value (key_file, G_KEY_FILE_DESKTOP_GROUP,
      ...
      else if (endo_str_is_equal (opt_type, G_KEY_FILE_DESKTOP_TYPE_DIRECTORY))
        {
          g_key_file_set_value (key_file, G_KEY_FILE_DESKTOP_GROUP,
      ...
      if (G_UNLIKELY (!res || length == 0))
        {
          ...
          if (G_LIKELY (error != NULL))
            {
              endo_die_error (_("Failed to load contents from \"%s\": %s"), argv[1], error->message);
              g_error_free (error);
            }
          else
            {
              endo_die_error (_("The file \"%s\" contains no data"), argv[1]);
            }
      ...
      if (G_UNLIKELY (!res))
        {
          ...
          endo_die_error (_("Failed to parse contents of \"%s\": %s"), argv[1], error->message);
          g_error_free (error);
          return EXIT_FAILURE;
        }
    }
  ...
  if (G_UNLIKELY (value == NULL))
    {
      ...
      endo_die_error (_("File \"%s\" has no type key"), argv[1]);
      return EXIT_FAILURE;
    }

  /* verify that we support the type */
  enum_klass = g_type_class_ref (ENDO_DIE_TYPE_EDITOR_MODE);
  enum_value = g_enum_get_value_by_nick (enum_klass, value);
  if (G_UNLIKELY (enum_value == NULL))
    {
      /* tell the user that we don't support the type */
      endo_die_error (_("Unsupported desktop file type \"%s\""), value);
      return EXIT_FAILURE;
    }
  g_free (value);
  ...
  g_signal_connect (G_OBJECT (dialog), "response",
                    G_CALLBACK (endo_die_response_cb), dialog);
  ...
  editor = endo_die_editor_new ();
  endo_die_editor_set_mode (ENDO_DIE_EDITOR (editor), mode);
  gtk_container_set_border_width (GTK_CONTAINER (editor), 12);
  endo_binding_new (G_OBJECT (editor), "complete", G_OBJECT (button), "sensitive");
  ...
  endo_die_editor_set_name (ENDO_DIE_EDITOR (editor), (value != NULL) ? value : "");
  g_free (value);

  /* setup the comment */
  value = g_key_file_get_locale_string (key_file, G_KEY_FILE_DESKTOP_GROUP,
                                        G_KEY_FILE_DESKTOP_KEY_COMMENT, NULL, NULL);
  endo_die_editor_set_comment (ENDO_DIE_EDITOR (editor), (value != NULL) ? value : "");
  g_free (value);
  ...
  endo_die_editor_set_icon (ENDO_DIE_EDITOR (editor), (value != NULL) ? value : "");
  g_free (value);
  ...
  switch (mode)
    ...
    case ENDO_DIE_EDITOR_MODE_APPLICATION:
      ...
      endo_die_editor_set_command (ENDO_DIE_EDITOR (editor), (value != NULL) ? value : "");
      g_free (value);

      /* setup the working directory */
      value = g_key_file_get_string (key_file, G_KEY_FILE_DESKTOP_GROUP,
                                     G_KEY_FILE_DESKTOP_KEY_PATH, NULL);
      endo_die_editor_set_path (ENDO_DIE_EDITOR (editor), (value != NULL) ? value : "");
      g_free (value);


      /* setup launcher options */
      endo_die_editor_set_snotify (ENDO_DIE_EDITOR (editor),
          g_key_file_get_boolean (key_file, G_KEY_FILE_DESKTOP_GROUP,
                                  G_KEY_FILE_DESKTOP_KEY_STARTUP_NOTIFY, NULL));
      endo_die_editor_set_terminal (ENDO_DIE_EDITOR (editor),
          g_key_file_get_boolean (key_file, G_KEY_FILE_DESKTOP_GROUP,
    ...
    case ENDO_DIE_EDITOR_MODE_LINK:
      ...
      endo_die_editor_set_url (ENDO_DIE_EDITOR (editor), (value != NULL) ? value : "");
      g_free (value);
      break;
  ...
  if (response == GTK_RESPONSE_ACCEPT)
    {
      ...
      endo_die_g_key_file_set_locale_value (key_file, G_KEY_FILE_DESKTOP_GROUP,
                                           G_KEY_FILE_DESKTOP_KEY_NAME,
                                           endo_die_editor_get_name (ENDO_DIE_EDITOR (editor)));
      endo_die_g_key_file_set_locale_value (key_file, G_KEY_FILE_DESKTOP_GROUP,
                                           G_KEY_FILE_DESKTOP_KEY_ICON,
                                           endo_die_editor_get_icon (ENDO_DIE_EDITOR (editor)));
      endo_die_g_key_file_set_locale_value (key_file, G_KEY_FILE_DESKTOP_GROUP,
                                           G_KEY_FILE_DESKTOP_KEY_COMMENT,
                                           endo_die_editor_get_comment (ENDO_DIE_EDITOR (editor)));
      ...
      switch (mode)
        ...
        case ENDO_DIE_EDITOR_MODE_APPLICATION:
          ...
          g_key_file_set_string (key_file, G_KEY_FILE_DESKTOP_GROUP,
                                 ...
                                 endo_die_editor_get_command (ENDO_DIE_EDITOR (editor)));

          /* save the new command */
          g_key_file_set_string (key_file, G_KEY_FILE_DESKTOP_GROUP,
                                 G_KEY_FILE_DESKTOP_KEY_PATH,
                                 endo_die_editor_get_path (ENDO_DIE_EDITOR (editor)));

          /* save the new launcher options */
          g_key_file_set_boolean (key_file, G_KEY_FILE_DESKTOP_GROUP,
                                 G_KEY_FILE_DESKTOP_KEY_TERMINAL,
                                 endo_die_editor_get_terminal (ENDO_DIE_EDITOR (editor)));
          g_key_file_set_boolean (key_file, G_KEY_FILE_DESKTOP_GROUP,
                                 G_KEY_FILE_DESKTOP_KEY_STARTUP_NOTIFY,
                                 endo_die_editor_get_snotify (ENDO_DIE_EDITOR (editor)));
          break;
        ...
        case ENDO_DIE_EDITOR_MODE_LINK:
          ...
          g_key_file_set_string (key_file, G_KEY_FILE_DESKTOP_GROUP,
                                 ...
                                 endo_die_editor_get_url (ENDO_DIE_EDITOR (editor)));
          break;
      ...
      if (!endo_die_g_key_file_save (key_file, opt_create_new, trusted_launcher, gfile, mode, &error))
        {
          if (opt_create_new)
            {
              /* reset the error */
              ...
              if (G_LIKELY (response == GTK_RESPONSE_ACCEPT))
                {
                  ...
                  endo_die_g_key_file_save (key_file, FALSE, trusted_launcher, gfile, mode, &error);
                }
            ...
            {
              if (mode == ENDO_DIE_EDITOR_MODE_DIRECTORY)
                  ...
                  if (G_LIKELY (path != NULL))
                    {
                      ...
                      endo_die_error ("\"%s\" is not writeable, saving to \"%s\" instead.",
                                     argv[1], path);
                      ...
                      endo_die_g_key_file_save (key_file, FALSE, trusted_launcher, gfile_local, mode, &error);
