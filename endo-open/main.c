 ...
 *
 * endo-open --launch WebBrowser https://expidus.org (bug #5461).
 * endo-open --launch WebBrowser https://expidus.org gitlab.expidus.org 'http://www.google.com/search?q=what is a space' 'https://wiki.expidus.org'
 * endo-open https://expidus.org
 * endo-open --launch TerminalEmulator ./script.sh 'something with a space' 'nospace' (bug #5132).
 * endo-open --launch TerminalEmulator ssh -l username some.host.com
 *
 * xfterm4 -e ssh -l username some.host.com (bug #5301, this generates line below)
 *   endo-open --launch TerminalEmulator 'ssh -l username some.host.com'
 *
 * endo-open /some/path/to/a/file.desktop
 * endo-open somerelativefile
 **/
...
usage (void)
{
  g_print ("%s\n", _("Usage: endo-open [URLs...]"));
  g_print ("%s\n", _("       endo-open --launch TYPE [PARAMETERs...]"));
  g_print ("\n");
  ...
  g_print ("\n");
  g_print ("%s\n", _("If you don't specify the --launch option, endo-open will open all specified\n"
...
static gboolean
endo_open_launch_desktop_file (const gchar *arg)
{
#ifdef HAVE_GIO_UNIX
  GFile           *gfile;
  gchar           *contents;
...
static gchar *
endo_open_get_path (const gchar *string)
{
  gchar *escaped;
  gchar *uri;
...
static gchar *
endo_open_find_scheme (const gchar *string)
{
  gchar *current_dir;
  gchar *uri;
  gchar *path;
  ...
  if (g_path_is_absolute (string))
    return endo_open_get_path (string);
   ...
   * endo_str_looks_like_an_uri() test */
...
static gboolean
endo_open_launch_category (const gchar *category,
                          const gchar *parameters)
{
  ...
  if (!endo_execute_preferred_application (category, parameters, opt_working_directory, NULL, &error))
    {
...
static gboolean
endo_open_uri_known_category (const gchar  *uri,
                             const gchar  *scheme,
                             gboolean     *succeed)
{
  guint        i;
      ...
      if (g_regex_match_simple (known_schemes[i].pattern, scheme, G_REGEX_CASELESS, 0))
        {
          ...
          *succeed = endo_open_launch_category (known_schemes[i].category, uri);
              ...
              GError      **error)
{
  ...
  if (scheme != NULL && endo_open_uri_known_category (uri, scheme, &retval))
    {
      g_free (scheme);
      return retval;
    }
      ...
      if (file_type == G_FILE_TYPE_DIRECTORY)
        {
          ...
          retval = endo_open_launch_category ("FileManager", uri);
          succeed = TRUE;
        }
      else
        {
          ...
          for (i = 0; schemes[i] != NULL; i++)
            {
              ...
              if (strcmp (scheme, schemes[i]) == 0)
                {
                  retval = succeed = endo_open_launch_category ("FileManager", uri);
                  break;
                }
            }
        }
  ...
  if (!succeed)
    {
      ...
      if (scheme != NULL
        ...
        retval = endo_open_launch_category ("WebBrowser", uri);
      else
  ...
  if (G_LIKELY (opt_launch != NULL))
    {
      ...
      /* run the preferred application */
      if (!endo_open_launch_category (opt_launch, parameter))
        result = EXIT_FAILURE;
      ...
      for (argv += 1; result == EXIT_SUCCESS && *argv != NULL; ++argv)
        {
          if (g_str_has_suffix (*argv, ".desktop")
              && endo_open_launch_desktop_file (*argv))
            {
              /* successfully launched a desktop file */
              continue;
            }
          else if (endo_str_looks_like_an_uri (*argv))
            {
          ...
          else
            {
              ...
              uri = endo_open_find_scheme (*argv);
            }
