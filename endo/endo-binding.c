 ...
 *            <link linkend="endo-Extensions-to-GObject">Extensions to GObject</link>
 *
 ...
 *   entry = gtk_entry_new ();
 *
 *   endo_binding_new (G_OBJECT (button), "active",
...
typedef struct
{
  ...
  gulong               handler;
  EndoBindingTransform  transform;
  gpointer             user_data;
}
EndoBindingLink;
 ...
 **/
struct _EndoMutualBinding
{
  GDestroyNotify  destroy;
  EndoBindingLink  direct;
  EndoBindingLink  reverse;
};



static void
endo_bind_properties_transfer (GObject             *src_object,
...
static void
endo_bind_properties_notify (GObject    *src_object,
                            GParamSpec *src_pspec,
                            gpointer    data)
{
  EndoBindingLink *blink = data;
  ...
  endo_bind_properties_transfer (src_object,
                                src_pspec,
                                blink->dst_object,
                                blink->dst_pspec,
                                blink->transform,
...
static void
endo_binding_on_dst_object_destroy (gpointer  data,
                                   GObject  *object)
{
  EndoBinding *binding = data;

  binding->blink.dst_object = NULL;

  /* calls endo_binding_on_disconnect() */
...
static void
endo_binding_on_disconnect (gpointer  data,
                           GClosure *closure)
{
  EndoBindingLink *blink = data;
  EndoBinding     *binding;

  binding = (EndoBinding *) (((gchar *) blink) - G_STRUCT_OFFSET (EndoBinding, blink));

  if (binding->destroy != NULL)
    binding->destroy (blink->user_data);

  if (blink->dst_object != NULL)
    g_object_weak_unref (blink->dst_object, endo_binding_on_dst_object_destroy, binding);

  g_slice_free (EndoBinding, binding);
}



/* recursively calls endo_mutual_binding_on_disconnect_object2() */
static void
endo_mutual_binding_on_disconnect_object1 (gpointer  data,
                                          GClosure *closure)
{
  EndoMutualBinding *binding;
  EndoBindingLink   *blink = data;
  GObject          *object2;

  binding = (EndoMutualBinding *) (((gchar *) blink) - G_STRUCT_OFFSET (EndoMutualBinding, direct));
  ...
  if (object2 != NULL)
    {
      ...
      g_slice_free (EndoMutualBinding, binding);
    }
}



/* recursively calls endo_mutual_binding_on_disconnect_object1() */
static void
endo_mutual_binding_on_disconnect_object2 (gpointer  data,
                                          GClosure *closure)
{
  EndoMutualBinding *binding;
  EndoBindingLink   *blink = data;
  ...
  binding = (EndoMutualBinding *) (((gchar *) blink) - G_STRUCT_OFFSET (EndoMutualBinding, reverse));
  binding->direct.dst_object = NULL;
...
static void
endo_binding_link_init (EndoBindingLink     *blink,
                       GObject            *src_object,
                       ...
                       gpointer            user_data)
{
  ...
  blink->handler = g_signal_connect_data (src_object,
                                          signal_name,
                                          G_CALLBACK (endo_bind_properties_notify),
                                          blink,
                                          destroy_notify,
                                          0);
  g_free (signal_name);
 ...
 **/
EndoBinding*
endo_binding_new (GObject      *src_object,
                 const gchar  *src_property,
                 GObject      *dst_object,
                 const gchar  *dst_property)
{
  return endo_binding_new_full (src_object, src_property,
 ...
 **/
EndoBinding*
endo_binding_new_full (GObject            *src_object,
                      const gchar        *src_property,
                      GObject            *dst_object,
                      const gchar        *dst_property,
                      EndoBindingTransform transform,
                      ...
                      gpointer            user_data)
{
  ...
  if (transform == NULL)
    transform = (EndoBindingTransform) (void (*)(void)) g_value_transform;
  ...
  binding = g_slice_new (EndoBinding);
  binding->src_object = src_object;
  binding->destroy = destroy_notify;

  endo_binding_link_init (&binding->blink,
                         src_object,
                         src_property,
                         dst_object,
                         dst_pspec,
                         transform,
                         endo_binding_on_disconnect,
                         user_data);

  g_object_weak_ref (dst_object, endo_binding_on_dst_object_destroy, binding);
...
/**
 * endo_binding_new_with_negation:
 * @src_object:   The source #GObject.
 * @src_property: The name of the property to bind from.
 * @dst_object:   The destination #GObject.
 * @dst_property: The name of the property to bind to.
 ...
 **/
EndoBinding*
endo_binding_new_with_negation (GObject      *src_object,
                               const gchar  *src_property,
                               GObject      *dst_object,
                               const gchar  *dst_property)
{
  EndoBindingTransform transform = (EndoBindingTransform) (void (*)(void)) endo_g_value_transform_negate;
...
/**
 * endo_binding_unbind:
 * @binding: An #EndoBinding to unbind.
 *
 ...
 **/
void
endo_binding_unbind (EndoBinding *binding)
{
  g_signal_handler_disconnect (binding->src_object, binding->blink.handler);
}
 ...
 **/
EndoMutualBinding*
endo_mutual_binding_new (GObject     *object1,
                        const gchar *property1,
                        GObject     *object2,
                        const gchar *property2)
{
  return endo_mutual_binding_new_full (object1, property1,
                                      object2, property2,
                                      NULL, NULL, NULL, NULL);
}
 ...
 **/
EndoMutualBinding*
endo_mutual_binding_new_full (GObject            *object1,
                             const gchar        *property1,
                             GObject            *object2,
                             const gchar        *property2,
                             EndoBindingTransform transform,
                             EndoBindingTransform reverse_transform,
                             GDestroyNotify      destroy_notify,
                             gpointer            user_data)
{
  EndoMutualBinding  *binding;
  ...
  if (reverse_transform == NULL)
    reverse_transform = (EndoBindingTransform) (void (*)(void)) g_value_transform;

  endo_bind_properties_transfer (object1,
                                pspec1,
                                object2,
                                pspec2,
                                transform,
  ...
  binding = g_slice_new (EndoMutualBinding);
  binding->destroy = destroy_notify;

  endo_binding_link_init (&binding->direct,
                         object1,
                         property1,
                         object2,
                         pspec2,
                         transform,
                         endo_mutual_binding_on_disconnect_object1,
                         user_data);

  endo_binding_link_init (&binding->reverse,
                         object2,
                         property2,
                         object1,
                         pspec1,
                         reverse_transform,
                         endo_mutual_binding_on_disconnect_object2,
                         user_data);
...
/**
 * endo_mutual_binding_new_with_negation:
 * @object1:   The first #GObject.
 * @property1: The first property to bind.
 * @object2:   The second #GObject.
 * @property2: The second property to bind.
 ...
 **/
EndoMutualBinding*
endo_mutual_binding_new_with_negation (GObject     *object1,
                                      const gchar *property1,
                                      GObject     *object2,
                                      const gchar *property2)
{
...
/**
 * endo_mutual_binding_unbind:
 * @binding: An #EndoMutualBinding to unbind.
 *
 * Disconnects the binding between two properties. Should be
 * rarely used by applications.
 *
 ...
 **/
void
endo_mutual_binding_unbind (EndoMutualBinding *binding)
{
