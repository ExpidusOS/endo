...
#include <endo/endo-thumbnail.h>
#include <endo/endo-alias.h>

/**
 * SECTION: endo-cell-renderer-icon
 * @title: EndoCellRendererIcon
 * @short_description: Renders an icon in a cell
 * @include: endo/endo.h
 * @see_also: <link linkend="EndoIconView">EndoIconView</link>
 *
 * An #EndoCellRendererIcon can be used to render an icon in a cell. It
 ...
 * <link linkend="EndoCellRendererIcon--icon">icon</link> property.
 *
 * To support the <link linkend="EndoIconView">EndoIconView</link> (and <link
 * linkend="GtkIconView">GtkIconView</link>), #EndoCellRendererIcon supports
 * rendering icons based on the state of the view if the
 * <link linkend="EndoCellRendererIcon--follow-state">follow-state</link>
 * property is set.
 **/
...
static void endo_cell_renderer_icon_finalize     (GObject                  *object);
static void endo_cell_renderer_icon_get_property (GObject                  *object,
                                                 guint                     prop_id,
                                                 GValue                   *value,
                                                 GParamSpec               *pspec);
static void endo_cell_renderer_icon_set_property (GObject                  *object,
                                                 guint                     prop_id,
                                                 const GValue             *value,
                                                 GParamSpec               *pspec);
static void endo_cell_renderer_icon_get_size     (GtkCellRenderer          *renderer,
                                                 GtkWidget                *widget,
                                                 ...
                                                 gint                     *width,
                                                 gint                     *height);
static void endo_cell_renderer_icon_render       (GtkCellRenderer          *renderer,
                                                 cairo_t                  *cr,
                                                 GtkWidget                *widget,
...
struct _EndoCellRendererIconPrivate
{
...
G_DEFINE_TYPE_WITH_PRIVATE (EndoCellRendererIcon, endo_cell_renderer_icon, GTK_TYPE_CELL_RENDERER)



static void
endo_cell_renderer_icon_class_init (EndoCellRendererIconClass *klass)
{
  GtkCellRendererClass *gtkcell_renderer_class;
  GObjectClass         *gobject_class;

  gobject_class = G_OBJECT_CLASS (klass);
  gobject_class->finalize = endo_cell_renderer_icon_finalize;
  gobject_class->get_property = endo_cell_renderer_icon_get_property;
  gobject_class->set_property = endo_cell_renderer_icon_set_property;

  gtkcell_renderer_class = GTK_CELL_RENDERER_CLASS (klass);
  gtkcell_renderer_class->get_size = endo_cell_renderer_icon_get_size;
  gtkcell_renderer_class->render = endo_cell_renderer_icon_render;

  /* initialize the library's i18n support */
  _endo_i18n_init ();

  /**
   * EndoCellRendererIcon:follow-state:
   *
   * Specifies whether the icon renderer should render icon based on the
   * selection state of the items. This is necessary for #EndoIconView,
...
static void
endo_cell_renderer_icon_init (EndoCellRendererIcon *icon_undocked)
{
}
...
static void
endo_cell_renderer_icon_finalize (GObject *object)
{
  const EndoCellRendererIconPrivate *priv = endo_cell_renderer_icon_get_instance_private (ENDO_CELL_RENDERER_ICON (object));
  ...
  (*G_OBJECT_CLASS (endo_cell_renderer_icon_parent_class)->finalize) (object);
}



static void
endo_cell_renderer_icon_get_property (GObject    *object,
                                     guint       prop_id,
                                     GValue     *value,
                                     GParamSpec *pspec)
{
...
static void
endo_cell_renderer_icon_set_property (GObject      *object,
                                     guint         prop_id,
                                     const GValue *value,
                                     GParamSpec   *pspec)
{
  EndoCellRendererIconPrivate *priv = endo_cell_renderer_icon_get_instance_private (ENDO_CELL_RENDERER_ICON (object));
  const gchar                *icon;
...
static void
endo_cell_renderer_icon_get_size (GtkCellRenderer *renderer,
                                 GtkWidget       *widget,
                                 const GdkRectangle *cell_area,
                                 gint            *x_offset,
                                 gint            *y_offset,
                                 ...
                                 gint            *height)
{
  const EndoCellRendererIconPrivate *priv = endo_cell_renderer_icon_get_instance_private (ENDO_CELL_RENDERER_ICON (renderer));
...
static void
endo_cell_renderer_icon_render (GtkCellRenderer     *renderer,
                               cairo_t             *cr,
                               GtkWidget           *widget,
                               const GdkRectangle  *background_area,
                               const GdkRectangle  *cell_area,
                               GtkCellRendererState flags)
{
  ...
  if (priv->icon != NULL && g_path_is_absolute (priv->icon))
    {
      ...
      icon = _endo_thumbnail_get_for_file (priv->icon, (priv->size > 128) ? ENDO_THUMBNAIL_SIZE_LARGE : ENDO_THUMBNAIL_SIZE_NORMAL, &err);
    }
      ...
      if (filename != NULL && g_str_has_suffix (filename, ".svg"))
        {
           ...
           */
          icon = _endo_thumbnail_get_for_file (filename, (priv->size > 128) ? ENDO_THUMBNAIL_SIZE_LARGE : ENDO_THUMBNAIL_SIZE_NORMAL, &err);
        }
      else
        {
  ...
  if (G_UNLIKELY (icon_area.width > cell_area->width || icon_area.height > cell_area->height))
    {
      /* scale down to fit */
      temp = endo_gdk_pixbuf_scale_down (icon, TRUE, cell_area->width, cell_area->height);
      ...
      if ((flags & (GTK_CELL_RENDERER_SELECTED | GTK_CELL_RENDERER_PRELIT)) != 0 && priv->follow_state)
        {
          if ((flags & GTK_CELL_RENDERER_SELECTED) != 0)
            {
              ...
              temp = endo_gdk_pixbuf_colorize (icon, &color_gdk);
              g_object_unref (G_OBJECT (icon));
              icon = temp;
            }
          ...
          if ((flags & GTK_CELL_RENDERER_PRELIT) != 0)
            {
              temp = endo_gdk_pixbuf_spotlight (icon);
              g_object_unref (G_OBJECT (icon));
              icon = temp;
            }
        }
...
/**
 * endo_cell_renderer_icon_new:
 *
 * Creates a new #EndoCellRendererIcon. Adjust rendering parameters using object properties,
 ...
 * you can bind the <link linkend="EndoCellRendererIcon--icon">icon</link> property on the
 * cell renderer to an icon name in the model, thus rendering a different icon in each row
 * of the #GtkTreeView.
 *
 * Returns: the newly allocated #EndoCellRendererIcon.
 *
 * Since: 0.3.1.9
 **/
GtkCellRenderer*
endo_cell_renderer_icon_new (void)
{
  return g_object_new (ENDO_TYPE_CELL_RENDERER_ICON, NULL);
}



#define __ENDO_CELL_RENDERER_ICON_C__
