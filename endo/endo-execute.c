 ...
 * application you should use endo_execute_terminal_shell() or
 * endo_execute_terminal_shell_on_screen() to make sure you run the user's
...
/**
 * endo_execute_preferred_application:
 ...
 *
 * Convenience wrapper to endo_execute_preferred_application_on_screen(), which
 ...
 **/
gboolean
endo_execute_preferred_application (const gchar *category,
                                   ...
                                   GError     **error)
{
  ...
  return endo_execute_preferred_application_on_screen (category, parameter, working_directory, envp, gdk_screen_get_default (), error);
}
...
find_fallback_application_from_xdg_mime (const gchar *category)
{
  ...
  if (info != NULL)
  {
    ...
    if (g_strcmp0(executable, "endo-open") != 0)
    {
...
/**
 * endo_execute_preferred_application_on_screen:
 * @category          : the category of the preferred application to launch.
 * @parameter         : additional parameter to pass to the preferred application
 *                      (i.e. an URL to pass to the preferred browser) or %NULL
 *                      to pass no parameter.
 ...
 *
 * libendo currently supports the following categories: %"WebBrowser",
 ...
 **/
gboolean
endo_execute_preferred_application_on_screen (const gchar *category,
                                             const gchar *parameter,
                                             const gchar *working_directory,
                                             gchar      **envp,
                                             GdkScreen   *screen,
 ...
 * @error             : return location for errors or %NULL.
 *
 * Convenience wrapper to endo_execute_terminal_shell_on_screen(), which
 ...
 **/
gboolean
endo_execute_terminal_shell (const gchar *command_line,
                            const gchar *working_directory,
                            gchar      **envp,
                            GError     **error)
{
  ...
  return endo_execute_terminal_shell_on_screen (command_line, working_directory, envp, gdk_screen_get_default (), error);
}



/**
 * endo_execute_terminal_shell_on_screen:
 * @command_line      : shell command line to execute.
 * @working_directory : path to the directory in which to execute @command_line
 *                      or %NULL to use the current working directory.
 * @envp              : child's environment, or %NULL to inherit parent's.
 ...
 **/
gboolean
endo_execute_terminal_shell_on_screen (const gchar *command_line,
                                      const gchar *working_directory,
                                      gchar      **envp,
                                      GdkScreen   *screen,
                                      GError     **error)
{
  ...
  return endo_execute_preferred_application_on_screen ("TerminalEmulator", command_line, working_directory, envp, screen, error);
}



#define __ENDO_EXECUTE_C__
