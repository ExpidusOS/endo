...
/**
 * SECTION: endo-icon-chooser-dialog
 * @title: EndoIconChooserDialog
...
static void     endo_icon_chooser_dialog_finalize                 (GObject                    *object);
static void     endo_icon_chooser_dialog_style_set                (GtkWidget                  *widget,
                                                                  GtkStyle                   *previous_style);
static void     endo_icon_chooser_dialog_screen_changed           (GtkWidget                  *widget,
                                                                  GdkScreen                  *previous_screen);
static void     endo_icon_chooser_dialog_close                    (GtkDialog                  *dialog);
static void     endo_icon_chooser_dialog_set_model                (EndoIconChooserDialog       *dialog);
static gboolean endo_icon_chooser_dialog_separator_func           (GtkTreeModel               *model,
                                                                  GtkTreeIter                *iter,
                                                                  gpointer                    user_data);
static gboolean endo_icon_chooser_dialog_visible_func             (GtkTreeModel               *model,
                                                                  GtkTreeIter                *iter,
                                                                  gpointer                    user_data);
static gboolean endo_icon_chooser_dialog_start_interactive_search (EndoIconChooserDialog       *icon_chooser_dialog);
static void     endo_icon_chooser_dialog_combo_changed            (GtkWidget                  *combo,
                                                                  EndoIconChooserDialog       *icon_chooser_dialog);
static void     endo_icon_chooser_dialog_entry_changed            (GtkWidget                  *entry,
                                                                  EndoIconChooserDialog       *icon_chooser_dialog);
static void     endo_icon_chooser_dialog_entry_clear              (GtkEntry                   *entry,
                                                                  GtkEntryIconPosition        icon_pos,
                                                                  GdkEvent                   *event);
static void     endo_icon_chooser_dialog_selection_changed        (EndoIconChooserDialog       *icon_chooser_dialog);



struct _EndoIconChooserDialogPrivate
{
  GtkWidget *combo;
...
static const gchar CONTEXT_TITLES[][80] =
{
  ...
  /* separator */
  "#",
  /* ENDO_ICON_CHOOSER_CONTEXT_ALL */
  N_("All Icons"),
  /* separator */
  "#",
  /* ENDO_ICON_CHOOSER_CONTEXT_FILE */
  N_("Image Files"),
  /* separator */
  "#",
  /* ENDO_ICON_CHOOSER_CONTEXT_NO_ICON */
  N_("No Icon"),
};



G_DEFINE_TYPE_WITH_PRIVATE (EndoIconChooserDialog, endo_icon_chooser_dialog, GTK_TYPE_DIALOG)



static void
endo_icon_chooser_dialog_class_init (EndoIconChooserDialogClass *klass)
{
  ...
  gobject_class->finalize = endo_icon_chooser_dialog_finalize;

  gtkwidget_class = GTK_WIDGET_CLASS (klass);
  gtkwidget_class->style_set = endo_icon_chooser_dialog_style_set;
  gtkwidget_class->screen_changed = endo_icon_chooser_dialog_screen_changed;

  gtkdialog_class = GTK_DIALOG_CLASS (klass);
  gtkdialog_class->close = endo_icon_chooser_dialog_close;
...
static void
endo_icon_chooser_dialog_init (EndoIconChooserDialog *icon_chooser_dialog)
{
  EndoIconChooserDialogPrivate *priv = endo_icon_chooser_dialog_get_instance_private (icon_chooser_dialog);
  ...
  gtk_combo_box_set_row_separator_func (GTK_COMBO_BOX (priv->combo), endo_icon_chooser_dialog_separator_func, icon_chooser_dialog, NULL);
  g_signal_connect (G_OBJECT (priv->combo), "changed", G_CALLBACK (endo_icon_chooser_dialog_combo_changed), icon_chooser_dialog);
  ...
  endo_binding_new (G_OBJECT (priv->filter_entry), "visible", G_OBJECT (label), "visible");
  ...
  g_signal_connect (G_OBJECT (priv->filter_entry), "changed", G_CALLBACK (endo_icon_chooser_dialog_entry_changed), icon_chooser_dialog);
  ...
  g_signal_connect (G_OBJECT (priv->filter_entry), "icon-release", G_CALLBACK (endo_icon_chooser_dialog_entry_clear), NULL);
  ...
  priv->icon_chooser = endo_icon_view_new ();
  endo_binding_new (G_OBJECT (priv->icon_chooser), "visible", G_OBJECT (scrolled_window), "visible");
  g_signal_connect_swapped (priv->icon_chooser, "item-activated", G_CALLBACK (gtk_window_activate_default), icon_chooser_dialog);
  g_signal_connect_swapped (priv->icon_chooser, "selection-changed", G_CALLBACK (endo_icon_chooser_dialog_selection_changed), icon_chooser_dialog);
  g_signal_connect_swapped (priv->icon_chooser, "start-interactive-search", G_CALLBACK (endo_icon_chooser_dialog_start_interactive_search), icon_chooser_dialog);
  ...
  renderer = endo_cell_renderer_icon_new ();
  gtk_cell_layout_pack_start (GTK_CELL_LAYOUT (priv->icon_chooser), renderer, TRUE);
  gtk_cell_layout_set_attributes (GTK_CELL_LAYOUT (priv->icon_chooser), renderer, "icon", ENDO_ICON_CHOOSER_MODEL_COLUMN_ICON_NAME, NULL);
  ...
  gtk_cell_layout_set_attributes (GTK_CELL_LAYOUT (priv->icon_chooser), renderer, "text", ENDO_ICON_CHOOSER_MODEL_COLUMN_ICON_NAME, NULL);
  ...
  endo_gtk_file_chooser_add_thumbnail_preview (GTK_FILE_CHOOSER (priv->file_chooser));
  ...
  g_signal_connect_swapped (priv->file_chooser, "selection-changed", G_CALLBACK (endo_icon_chooser_dialog_selection_changed), icon_chooser_dialog);
  ...
  /* update the model */
  endo_icon_chooser_dialog_set_model (icon_chooser_dialog);

  /* default to "Application Icons", since thats what users probably expect to see */
  gtk_combo_box_set_active (GTK_COMBO_BOX (priv->combo), ENDO_ICON_CHOOSER_CONTEXT_APPLICATIONS);
}



static void
endo_icon_chooser_dialog_finalize (GObject  *object)
{
  EndoIconChooserDialogPrivate *priv = endo_icon_chooser_dialog_get_instance_private (ENDO_ICON_CHOOSER_DIALOG (object));

  g_free (priv->casefolded_text);

  (*G_OBJECT_CLASS (endo_icon_chooser_dialog_parent_class)->finalize) (object);
}



static void
endo_icon_chooser_dialog_style_set (GtkWidget *widget,
                                   GtkStyle  *previous_style)
{
  /* call the parent's style_set method */
  if (GTK_WIDGET_CLASS (endo_icon_chooser_dialog_parent_class)->style_set != NULL)
    (*GTK_WIDGET_CLASS (endo_icon_chooser_dialog_parent_class)->style_set) (widget, previous_style);

  /* update the model */
  endo_icon_chooser_dialog_set_model (ENDO_ICON_CHOOSER_DIALOG (widget));
}



static void
endo_icon_chooser_dialog_screen_changed (GtkWidget *widget,
                                        GdkScreen *previous_screen)
{
  /* call the parent's screen_changed method */
  if (GTK_WIDGET_CLASS (endo_icon_chooser_dialog_parent_class)->screen_changed != NULL)
    (*GTK_WIDGET_CLASS (endo_icon_chooser_dialog_parent_class)->screen_changed) (widget, previous_screen);
...
static void
endo_icon_chooser_dialog_close (GtkDialog *dialog)
{
  GdkEvent *event;
...
static void
endo_icon_chooser_dialog_set_model (EndoIconChooserDialog *dialog)
{
  EndoIconChooserDialogPrivate *priv = endo_icon_chooser_dialog_get_instance_private (ENDO_ICON_CHOOSER_DIALOG (dialog));
  EndoIconChooserModel         *model;
  GtkTreeModel                *filter;

  /* determine the icon chooser model for the widget */
  model = _endo_icon_chooser_model_get_for_widget (GTK_WIDGET (dialog));

  /* check if we have a new model here */
  filter = endo_icon_view_get_model (ENDO_ICON_VIEW (priv->icon_chooser));
  if (filter == NULL
      || GTK_TREE_MODEL (model) != gtk_tree_model_filter_get_model (GTK_TREE_MODEL_FILTER (filter)))
    {
      ...
      gtk_tree_model_filter_set_visible_func (GTK_TREE_MODEL_FILTER (filter), endo_icon_chooser_dialog_visible_func, dialog, NULL);
      endo_icon_view_set_model (ENDO_ICON_VIEW (priv->icon_chooser), filter);
      g_object_unref (G_OBJECT (filter));

      /* enable search on the display name */
      endo_icon_view_set_search_column (ENDO_ICON_VIEW (priv->icon_chooser), ENDO_ICON_CHOOSER_MODEL_COLUMN_ICON_NAME);
    }
...
static gboolean
endo_icon_chooser_dialog_separator_func (GtkTreeModel *model,
                                        GtkTreeIter  *iter,
                                        gpointer      user_data)
{
  gboolean separator;
...
static gboolean
endo_icon_chooser_dialog_visible_func (GtkTreeModel *model,
                                      GtkTreeIter  *iter,
                                      gpointer      user_data)
{
  EndoIconChooserDialogPrivate *priv = endo_icon_chooser_dialog_get_instance_private (ENDO_ICON_CHOOSER_DIALOG (user_data));
  ...
  if (G_LIKELY (icon_chooser_context < ENDO_ICON_CHOOSER_CONTEXT_ALL))
    {
      /* determine the context for the iter... */
      gtk_tree_model_get (model, iter, ENDO_ICON_CHOOSER_MODEL_COLUMN_CONTEXT, &item_context, -1);
  ...
  if (priv->casefolded_text != NULL)
    {
      gtk_tree_model_get (model, iter, ENDO_ICON_CHOOSER_MODEL_COLUMN_ICON_NAME, &name, -1);
...
static gboolean
endo_icon_chooser_dialog_start_interactive_search (EndoIconChooserDialog *icon_chooser_dialog)
{
  EndoIconChooserDialogPrivate *priv = endo_icon_chooser_dialog_get_instance_private (icon_chooser_dialog);
...
static void
endo_icon_chooser_dialog_combo_changed (GtkWidget            *combo,
                                       EndoIconChooserDialog *icon_chooser_dialog)
{
  EndoIconChooserDialogPrivate *priv = endo_icon_chooser_dialog_get_instance_private (icon_chooser_dialog);
  EndoIconChooserContext        context;
  ...
  if (context <= ENDO_ICON_CHOOSER_CONTEXT_ALL)
    {
      ...
      model = endo_icon_view_get_model (ENDO_ICON_VIEW (priv->icon_chooser));
      if (G_LIKELY (model != NULL))
        gtk_tree_model_filter_refilter (GTK_TREE_MODEL_FILTER (model));

      /* check if the icon chooser has a selected item */
      selected_items = endo_icon_view_get_selected_items (ENDO_ICON_VIEW (priv->icon_chooser));
      if (G_LIKELY (selected_items != NULL))
        {
          /* make sure the selected item is visible */
          endo_icon_view_scroll_to_path (ENDO_ICON_VIEW (priv->icon_chooser), selected_items->data, FALSE, 0.0f, 0.0f);
          g_list_foreach (selected_items, (GFunc) (void (*)(void)) gtk_tree_path_free, NULL);
          g_list_free (selected_items);
        }
    }
  else if (context == ENDO_ICON_CHOOSER_CONTEXT_FILE)
    {
  ...
  endo_icon_chooser_dialog_selection_changed (icon_chooser_dialog);
}



static void
endo_icon_chooser_dialog_entry_changed (GtkWidget            *combo,
                                       EndoIconChooserDialog *icon_chooser_dialog)
{
  EndoIconChooserDialogPrivate *priv = endo_icon_chooser_dialog_get_instance_private (icon_chooser_dialog);
  const gchar                 *text;
...
static void
endo_icon_chooser_dialog_entry_clear (GtkEntry             *entry,
                                     GtkEntryIconPosition  icon_pos,
                                     GdkEvent             *event)
{
...
static void
endo_icon_chooser_dialog_selection_changed (EndoIconChooserDialog *icon_chooser_dialog)
{
  gchar *icon;

  /* check if we have a valid icon in the chooser */
  icon = endo_icon_chooser_dialog_get_icon (icon_chooser_dialog);
 ...
 *                      ending with %NULL.
 *
 * Creates a new #EndoIconChooserDialog. This function is analogous to gtk_dialog_new_with_buttons().
 *
 * Returns: a new #EndoIconChooserDialog.
 *
 * Since: 0.3.1.9
 **/
GtkWidget*
endo_icon_chooser_dialog_new (const gchar *title,
                             GtkWindow   *parent,
                             ...
                             ...)
{
  ...
  dialog = g_object_new (ENDO_TYPE_ICON_CHOOSER_DIALOG,
                         "title", title,
                         NULL);
...
/**
 * endo_icon_chooser_dialog_get_icon:
 * @icon_chooser_dialog : an #EndoIconChooserDialog.
 *
 ...
 **/
gchar*
endo_icon_chooser_dialog_get_icon (EndoIconChooserDialog *icon_chooser_dialog)
{
  EndoIconChooserDialogPrivate *priv = endo_icon_chooser_dialog_get_instance_private (icon_chooser_dialog);
  EndoIconChooserContext        context;
  GtkTreeModel                *model;
  ...
  g_return_val_if_fail (ENDO_IS_ICON_CHOOSER_DIALOG (icon_chooser_dialog), NULL);
      ...
      if (G_LIKELY (selected_items != NULL))
        {
          ...
          if (gtk_tree_model_get_iter (model, &iter, selected_items->data))
            gtk_tree_model_get (model, &iter, ENDO_ICON_CHOOSER_MODEL_COLUMN_ICON_NAME, &icon, -1);
...
/**
 * endo_icon_chooser_dialog_set_icon:
 * @icon_chooser_dialog : an #EndoIconChooserDialog.
 ...
 **/
gboolean
endo_icon_chooser_dialog_set_icon (EndoIconChooserDialog *icon_chooser_dialog,
                                  const gchar          *icon)
{
  EndoIconChooserDialogPrivate *priv = endo_icon_chooser_dialog_get_instance_private (icon_chooser_dialog);
  GtkTreeModel                *filter;
  ...
  g_return_val_if_fail (ENDO_IS_ICON_CHOOSER_DIALOG (icon_chooser_dialog), FALSE);
  ...
  if (g_path_is_absolute (icon))
      ...
      if (gtk_file_chooser_set_filename (GTK_FILE_CHOOSER (priv->file_chooser), icon))
        {
          ...
          gtk_combo_box_set_active (GTK_COMBO_BOX (priv->combo), ENDO_ICON_CHOOSER_CONTEXT_FILE);
          return TRUE;
        }
    }
  else
    {
      ...
      if (_endo_icon_chooser_model_get_iter_for_icon_name (ENDO_ICON_CHOOSER_MODEL (model), &model_iter, icon))
        {
              ...
              if (G_UNLIKELY (filter_path == NULL))
                {
                  ...
                  gtk_tree_model_get (model, &model_iter, ENDO_ICON_CHOOSER_MODEL_COLUMN_CONTEXT, &context, -1);
              ...
              if (G_LIKELY (filter_path != NULL))
                {
                  ...
                  endo_icon_view_select_path (ENDO_ICON_VIEW (priv->icon_chooser), filter_path);
                  endo_icon_view_set_cursor (ENDO_ICON_VIEW (priv->icon_chooser), filter_path, NULL, FALSE);
...
#define __ENDO_ICON_CHOOSER_DIALOG_C__
