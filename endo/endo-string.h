...
#ifndef __ENDO_STRING_H__
#define __ENDO_STRING_H__

#include <endo/endo-config.h>

G_BEGIN_DECLS

gchar                *endo_str_elide_underscores  (const gchar     *text) G_GNUC_MALLOC G_GNUC_WARN_UNUSED_RESULT;

gboolean              endo_str_is_equal           (const gchar     *a,
                                                  const gchar     *b);

gchar                *endo_str_replace            (const gchar     *str,
                                                  const gchar     *pattern,
                                                  const gchar     *replacement) G_GNUC_MALLOC G_GNUC_WARN_UNUSED_RESULT;

gchar                *endo_strdup_strftime        (const gchar     *format,
                                                  const struct tm *tm) G_GNUC_MALLOC G_GNUC_WARN_UNUSED_RESULT;

gchar               **endo_strndupv               (gchar          **strv,
                                                  guint            num) G_GNUC_MALLOC G_GNUC_WARN_UNUSED_RESULT;

gboolean              endo_str_looks_like_an_uri  (const gchar     *str);

gboolean              endo_str_is_flag            (const gchar     *str);
 ...
 * Since : 0.5.0
 **/
#define endo_str_is_empty(string) ((string) == NULL || *(string) == '\0')
