...
#include <endo/endo-thumbnail-preview.h>
...
static void endo_thumbnail_preview_style_set (GtkWidget           *ebox,
                                             GtkStyle            *previous_style,
                                             EndoThumbnailPreview *thumbnail_preview);



struct _EndoThumbnailPreviewClass
{
  GtkFrameClass __parent__;
};

struct _EndoThumbnailPreview
{
  GtkFrame   __parent__;
  GtkWidget *image;
...
G_DEFINE_TYPE (EndoThumbnailPreview, endo_thumbnail_preview, GTK_TYPE_FRAME)



static void
endo_thumbnail_preview_class_init (EndoThumbnailPreviewClass *klass)
{
}
...
static void
endo_thumbnail_preview_init (EndoThumbnailPreview *thumbnail_preview)
{
  GtkWidget *button;
  GtkWidget *label;
  GtkWidget *ebox;
  ...
  g_signal_connect (G_OBJECT (ebox), "style-set", G_CALLBACK (endo_thumbnail_preview_style_set), thumbnail_preview);
  ...
  g_signal_connect (G_OBJECT (button), "button-press-event", G_CALLBACK (endo_noop_true), NULL);
  g_signal_connect (G_OBJECT (button), "button-release-event", G_CALLBACK (endo_noop_true), NULL);
  g_signal_connect (G_OBJECT (button), "enter-notify-event", G_CALLBACK (endo_noop_true), NULL);
  g_signal_connect (G_OBJECT (button), "leave-notify-event", G_CALLBACK (endo_noop_true), NULL);
  ...
  gtk_widget_set_size_request (thumbnail_preview->image, ENDO_THUMBNAIL_SIZE_NORMAL + 2 * 12, ENDO_THUMBNAIL_SIZE_NORMAL + 2 * 12);
  gtk_image_set_pixel_size (GTK_IMAGE (thumbnail_preview->image), ENDO_THUMBNAIL_SIZE_NORMAL / 2);
...
static void
endo_thumbnail_preview_style_set (GtkWidget           *ebox,
                                 GtkStyle            *previous_style,
                                 EndoThumbnailPreview *thumbnail_preview)
{
  _endo_return_if_fail (ENDO_IS_THUMBNAIL_PREVIEW (thumbnail_preview));
  _endo_return_if_fail (GTK_IS_EVENT_BOX (ebox));

  /* check if the ebox is already realized */
  if (gtk_widget_get_realized (ebox))
    {
      /* set background color (using the base color) */
      g_signal_handlers_block_by_func (G_OBJECT (ebox), endo_thumbnail_preview_style_set, thumbnail_preview);
      g_signal_handlers_unblock_by_func (G_OBJECT (ebox), endo_thumbnail_preview_style_set, thumbnail_preview);
    }
}
...
/**
 * _endo_thumbnail_preview_new:
 *
 * Allocates a new #EndoThumbnailPreview instance.
 *
 * Returns: the newly allocated #EndoThumbnailPreview.
 **/
GtkWidget*
_endo_thumbnail_preview_new (void)
{
  return g_object_new (ENDO_TYPE_THUMBNAIL_PREVIEW, NULL);
}
...
thumbnail_add_frame (GdkPixbuf *thumbnail)
{
  ...
  if (width < ENDO_THUMBNAIL_SIZE_NORMAL && height < ENDO_THUMBNAIL_SIZE_NORMAL)
    goto none;
  ...
  frame = gdk_pixbuf_new_from_file (DATADIR G_DIR_SEPARATOR_S "pixmaps" G_DIR_SEPARATOR_S "endo"
                                    G_DIR_SEPARATOR_S "endo-thumbnail-frame.png", NULL);
  if (G_LIKELY (frame != NULL))
    {
      /* add a frame to the thumbnail */
      thumbnail = endo_gdk_pixbuf_frame (thumbnail, frame, 4, 3, 5, 6);
...
/**
 * _endo_thumbnail_preview_set_uri:
 * @thumbnail_preview : an #EndoThumbnailPreview.
 ...
 **/
void
_endo_thumbnail_preview_set_uri (EndoThumbnailPreview *thumbnail_preview,
                                const gchar         *uri)
{
  struct stat statb;
  ...
  else
    {
      ...
      if (G_LIKELY (filename != NULL))
      ...
      else
        {
          ...
          slash = strrchr (uri, '/');
          if (G_LIKELY (!endo_str_is_empty (slash)))
      ...
      if (G_UNLIKELY (icon_name != NULL))
      ...
      else
        {
          ...
          thumbnail = _endo_thumbnail_get_for_uri (uri, ENDO_THUMBNAIL_SIZE_NORMAL, NULL);
          if (thumbnail == NULL && G_LIKELY (filename != NULL))
            {
              /* but we can try to generate a thumbnail */
              thumbnail = _endo_thumbnail_get_for_file (filename, ENDO_THUMBNAIL_SIZE_NORMAL, NULL);
            }
...
#define __ENDO_THUMBNAIL_PREVIEW_C__
