...
/**
 * SECTION: endo-tree-view
 * @title: EndoTreeView
 * @short_description: An improved version of #GtkTreeView
 * @include: endo/endo.h
 *
 * The #EndoTreeView class derives from #GtkTreeView and extends it with
 ...
 * of #GtkTreeView, i.e. #EndoTreeView allows the user to drag around multiple
 * selected rows.
 **/
...
static void     endo_tree_view_finalize                      (GObject          *object);
static void     endo_tree_view_get_property                  (GObject          *object,
                                                             guint             prop_id,
                                                             GValue           *value,
                                                             GParamSpec       *pspec);
static void     endo_tree_view_set_property                  (GObject          *object,
                                                             guint             prop_id,
                                                             const GValue     *value,
                                                             GParamSpec       *pspec);
static gboolean endo_tree_view_button_press_event            (GtkWidget        *widget,
                                                             GdkEventButton   *event);
static gboolean endo_tree_view_button_release_event          (GtkWidget        *widget,
                                                             GdkEventButton   *event);
static gboolean endo_tree_view_motion_notify_event           (GtkWidget        *widget,
                                                             GdkEventMotion   *event);
static gboolean endo_tree_view_leave_notify_event            (GtkWidget        *widget,
                                                             GdkEventCrossing *event);
static void     endo_tree_view_drag_begin                    (GtkWidget        *widget,
                                                             GdkDragContext   *context);
static gboolean endo_tree_view_move_cursor                   (GtkTreeView      *view,
                                                             GtkMovementStep   step,
                                                             gint              count);
static gboolean endo_tree_view_single_click_timeout          (gpointer          user_data);
static void     endo_tree_view_single_click_timeout_destroy  (gpointer          user_data);



struct _EndoTreeViewPrivate
{
...
G_DEFINE_TYPE_WITH_PRIVATE (EndoTreeView, endo_tree_view, GTK_TYPE_TREE_VIEW)



static void
endo_tree_view_class_init (EndoTreeViewClass *klass)
{
  ...
  gobject_class->finalize = endo_tree_view_finalize;
  gobject_class->get_property = endo_tree_view_get_property;
  gobject_class->set_property = endo_tree_view_set_property;

  gtkwidget_class = GTK_WIDGET_CLASS (klass);
  gtkwidget_class->button_press_event = endo_tree_view_button_press_event;
  gtkwidget_class->button_release_event = endo_tree_view_button_release_event;
  gtkwidget_class->motion_notify_event = endo_tree_view_motion_notify_event;
  gtkwidget_class->leave_notify_event = endo_tree_view_leave_notify_event;
  gtkwidget_class->drag_begin = endo_tree_view_drag_begin;

  gtktree_view_class = GTK_TREE_VIEW_CLASS (klass);
  gtktree_view_class->move_cursor = endo_tree_view_move_cursor;

  /* initialize the library's i18n support */
  _endo_i18n_init ();

  /**
   * EndoTreeView:single-click:
   *
  ...
  /**
   * EndoTreeView:single-click-timeout:
   *
...
static void
endo_tree_view_init (EndoTreeView *tree_view)
{
  /* grab a pointer on the private data */
  tree_view->priv = endo_tree_view_get_instance_private (tree_view);
...
static void
endo_tree_view_finalize (GObject *object)
{
  EndoTreeView *tree_view = ENDO_TREE_VIEW (object);
  ...
  (*G_OBJECT_CLASS (endo_tree_view_parent_class)->finalize) (object);
}



static void
endo_tree_view_get_property (GObject    *object,
                            guint       prop_id,
                            GValue     *value,
                            GParamSpec *pspec)
{
  ...
  switch (prop_id)
    ...
    case PROP_SINGLE_CLICK:
      g_value_set_boolean (value, endo_tree_view_get_single_click (tree_view));
      break;

    case PROP_SINGLE_CLICK_TIMEOUT:
      g_value_set_uint (value, endo_tree_view_get_single_click_timeout (tree_view));
      break;
...
static void
endo_tree_view_set_property (GObject      *object,
                            guint         prop_id,
                            const GValue *value,
                            GParamSpec   *pspec)
{
  ...
  switch (prop_id)
    {
    case PROP_SINGLE_CLICK:
      endo_tree_view_set_single_click (tree_view, g_value_get_boolean (value));
      break;

    case PROP_SINGLE_CLICK_TIMEOUT:
      endo_tree_view_set_single_click_timeout (tree_view, g_value_get_uint (value));
      break;
...
static gboolean
endo_tree_view_button_press_event (GtkWidget      *widget,
                                  GdkEventButton *event)
{
  GtkTreeSelection *selection;
  EndoTreeView      *tree_view = ENDO_TREE_VIEW (widget);
  ...
  if (event->type == GDK_BUTTON_PRESS && (event->state & gtk_accelerator_get_default_mod_mask ()) == 0
    ...
    {
      gtk_tree_selection_set_select_function (selection, (GtkTreeSelectionFunc) (void (*)(void)) endo_noop_false, NULL, NULL);
    }

  /* call the parent's button press handler */
  result = (*GTK_WIDGET_CLASS (endo_tree_view_parent_class)->button_press_event) (widget, event);
  ...
  if (G_LIKELY (gtk_tree_selection_get_select_function (selection) == (GtkTreeSelectionFunc) (void (*)(void)) endo_noop_false))
    {
      gtk_tree_selection_set_select_function (selection, (GtkTreeSelectionFunc) (void (*)(void)) endo_noop_true, NULL, NULL);
    }
...
static gboolean
endo_tree_view_button_release_event (GtkWidget      *widget,
                                    GdkEventButton *event)
{
  ...
  return (*GTK_WIDGET_CLASS (endo_tree_view_parent_class)->button_release_event) (widget, event);
}



static gboolean
endo_tree_view_motion_notify_event (GtkWidget      *widget,
                                   GdkEventMotion *event)
{
  EndoTreeView *tree_view = ENDO_TREE_VIEW (widget);
  GtkTreePath *path;
              ...
              if (G_LIKELY (tree_view->priv->single_click_timeout > 0 && tree_view->priv->hover_path != NULL))
                {
                  ...
                  tree_view->priv->single_click_timeout_id = gdk_threads_add_timeout_full (G_PRIORITY_LOW, tree_view->priv->single_click_timeout,
                                                                                           endo_tree_view_single_click_timeout, tree_view,
                                                                                           endo_tree_view_single_click_timeout_destroy);
                }
            }
          else
            {
  ...
  return (*GTK_WIDGET_CLASS (endo_tree_view_parent_class)->motion_notify_event) (widget, event);
}



static gboolean
endo_tree_view_leave_notify_event (GtkWidget        *widget,
                                  GdkEventCrossing *event)
{
  EndoTreeView *tree_view = ENDO_TREE_VIEW (widget);
  ...
  return (*GTK_WIDGET_CLASS (endo_tree_view_parent_class)->leave_notify_event) (widget, event);
}



static void
endo_tree_view_drag_begin (GtkWidget      *widget,
                          GdkDragContext *context)
{
  EndoTreeView *tree_view = ENDO_TREE_VIEW (widget);
  ...
  (*GTK_WIDGET_CLASS (endo_tree_view_parent_class)->drag_begin) (widget, context);
}



static gboolean
endo_tree_view_move_cursor (GtkTreeView    *view,
                           GtkMovementStep step,
                           gint            count)
{
  EndoTreeView *tree_view = ENDO_TREE_VIEW (view);
  ...
  return (*GTK_TREE_VIEW_CLASS (endo_tree_view_parent_class)->move_cursor) (view, step, count);
}



static gboolean
endo_tree_view_single_click_timeout (gpointer user_data)
{
  ...
  GtkTreeIter        iter;
  EndoTreeView       *tree_view = ENDO_TREE_VIEW (user_data);
...
static void
endo_tree_view_single_click_timeout_destroy (gpointer user_data)
{
  ENDO_TREE_VIEW (user_data)->priv->single_click_timeout_id = -1;
}
 ...
 * endo_tree_view_new:
 *
 * Allocates a new #EndoTreeView instance.
 *
 * Returns: the newly allocated #EndoTreeView.
 *
 * Since: 0.3.1.3
 **/
GtkWidget*
endo_tree_view_new (void)
{
  return g_object_new (ENDO_TYPE_TREE_VIEW, NULL);
}



/**
 * endo_tree_view_get_single_click:
 * @tree_view : an #EndoTreeView.
 *
 ...
 **/
gboolean
endo_tree_view_get_single_click (const EndoTreeView *tree_view)
{
  g_return_val_if_fail (ENDO_IS_TREE_VIEW (tree_view), FALSE);
...
/**
 * endo_tree_view_set_single_click:
 * @tree_view    : an #EndoTreeView.
 ...
 **/
void
endo_tree_view_set_single_click (EndoTreeView *tree_view,
                                gboolean     single_click)
{
  g_return_if_fail (ENDO_IS_TREE_VIEW (tree_view));
...
/**
 * endo_tree_view_get_single_click_timeout:
 * @tree_view : a #EndoTreeView.
 *
 ...
 **/
guint
endo_tree_view_get_single_click_timeout (const EndoTreeView *tree_view)
{
  g_return_val_if_fail (ENDO_IS_TREE_VIEW (tree_view), 0u);
...
/**
 * endo_tree_view_set_single_click_timeout:
 * @tree_view            : a #EndoTreeView.
 ...
 * single-click mode, see endo_tree_view_set_single_click().
 *
 * Since: 0.3.1.5
 **/
void
endo_tree_view_set_single_click_timeout (EndoTreeView *tree_view,
                                        guint        single_click_timeout)
{
  g_return_if_fail (ENDO_IS_TREE_VIEW (tree_view));
